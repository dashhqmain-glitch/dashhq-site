...
-- Not self-computed (that's nft_smart_wallets/nft_wallet_pnl_stats above) -
-- this is an EXTERNAL, manually-curated list staff imports via
-- /smart-wallets import (e.g. a scraped GMGN/OpenSea leaderboard), one row
-- per (wallet, credential) since a single wallet routinely carries several
-- tags at once ("Early FEFER @$181k", "Top 6 REALCOIN"). Feeds NFT Scope's
-- scoring as one more signal (_nft_scope_tracked_wallet_hits) and enriches
-- /xray - never posts anything on its own.
create table if not exists smart_wallet_tags (
  address      text not null,  -- lowercased 0x address
  tag          text not null,  -- community/collection/token label, e.g. "RH MACHINES", "REALCOIN"
  rank         int,
  pnl          numeric,
  source       text,
